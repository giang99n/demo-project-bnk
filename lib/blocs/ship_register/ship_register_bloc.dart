...
class ShipRegisterBloc extends Bloc<ShipRegisterEvents, ShipRegisterState> {
  ...
  ShipRegisterBloc(ShipRegisterState initialState, this.apiRepository)
      : super(initialState);
  ...
  Stream<ShipRegisterState> mapEventToState(ShipRegisterEvents event) async* {
    ...
    if (event is StartEvent) {
      ...
      var data = await apiRepository.shipRegister(event.orderName, event.value,
          event.time, event.isChecked, event.deliveryTime,
          token: token);
      if (data != null) {
        if (data!.status == 1) {
          yield ShipRegisterSuccessState();
        } else if (data.status == 0) {
          yield ShipRegisterErrorState(
