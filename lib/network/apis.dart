...
class Api {
  ...
  Future<RegisterResidentResponse?> RegisterResident(
    ...
    try {
      response = await dio.post(
          'http://192.168.0.176:5000/api/v1/auths/resident/register',
          data: {'name': name});
      if (response.statusCode == 200) {
  ...
  Future<GetUserResponse?> getUser(String token) async {
    ...
    try {
      response = await dio.get(
        'http://192.168.0.176:5000/api/v1/auths/verify',
      );
      if (response.statusCode == 200) {
  ...
  Future<BillResponse?> getPaidBill(String token) async {
    ...
    try {
      response = await dio.get(
        'http://192.168.0.176:5000/api/v1/auths/bill',
      );
      if (response.statusCode == 200) {
  ...
  Future<ShipListOrderResponse?> getShipListOrder(String token) async {
    ...
    try {
      response = await dio.get(
        'http://192.168.0.176:5000/api/v1/auths/ship/list',
      );
      if (response.statusCode == 200) {
        return ShipListOrderResponse.fromJson(response.data);
      } else {
        print('There is some problem status code not 200');
      }
    } on Exception catch (e) {
  ...
  Future<ShipRegisterResponse?> shipRegister(String orderName, String value,
      String time, bool isChecked, String deliveryTime,
    ...
    try {
      print(123);
      response = await dio
          .post('http://192.168.0.176:5000/api/v1/auths/ship/register', data: {
        'orderName': orderName,
        'value': value,
        'time': time,
        'isChecked': isChecked,
